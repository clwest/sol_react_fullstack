...
class App extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      this.setState({ loaded: true, tokenSaleAddress: this.myTokenSale._address, updateUserTokens:this.updateUserTokens });
    } catch (error) {
  ...
  render() {
    ...
    return (
      <div className="App">
        <h1>Lets build some coins!!</h1>


        <h2>Enable your account</h2>
        Address to allow: <input type="text" name="kycAddress" value={this.state.kycAddress} onChange={this.handleInputChange} />
        <button type="button" onClick={this.handleKycSubmit}>Add Address To Wallet</button>
        <h2>Buy Tokens</h2>
        <p>If you want to buy tokens, send Wei to this address: {this.state.tokenSaleAddress}</p>
        <p>You have: { this.state.userTokens } WOW Tokens</p>
        <button type="button" onClick={this.handleBuyToken}>Buy Tokens</button>
      </div>
    );
  }
}
